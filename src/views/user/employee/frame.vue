...
export default {
    // 引入的组件列表
    components: {
        infoBasic, employeeSummary, infoPassword, infoAvatar, infoProfile,infoYunxin
    },
    data() {
        return {
    ...
    methods: {
        ...
        checkUser(data) {
            if (data.profile == null || !_.has(data.profile, 'employee_id')) {
                helper.message('error', '该用户不是一个员工或无员工号!');
                return false;
            }
            return data;
        }
