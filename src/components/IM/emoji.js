...
function buildEmoji(text) {
	...
	for (var j = 0, len = matches.length; j < len; ++j) {
		if(emoji[matches[j]]){
			text = text.replace(matches[j], '<img class="emoji" src="/assets/images/emoji/' + emoji[matches[j]].file + '" />');
		}		
	}
	return text;
}
...
CEmojiEngine.prototype.__renderChangeCol = function(){
	for(var i in this._emojiList){
		...
		for(var key in emojiList){
			...
			span.id = 'chn-emoji-'+i;	
			var img = new Image();
			img.src = this._imgpath +'emoji'+'/'+emojiList[key]['file'];
	...
	for(var i in this._pinupList){
		...
		span.id = 'chn-pinup-'+i;
		var img = new Image();
		img.src = this._imgpath +'emoji'+'/'+ pinupList[0];
		span.appendChild(img);
		this._changeColumn.appendChild(span);		
	}
}
...
CEmojiEngine.prototype.__renderPictureCol= function(){
	...
	if(isEmoji){		//表情包
		...
		for(var key in emojiList){
			...
			img.style.width=this._lWidth+'px'; // 暂时
			span.appendChild(img);
...
CEmojiEngine.prototype._$show = function(){
	if(this._parentNode.style.display === 'block' || this._parentNode.style.display === '') return;
	this._curEmojiType = 'emoji-emoji';	//初始化当前默认表情选择
	this._curEmojiNode = document.getElementById('chn-emoji-emoji');
  this.__renderPictureCol();		//渲染表情图片栏
...
export {emoji,emojiList,CEmojiEngine};
